...
rrdtool graph $RRDGRAPHINC -a PNG --title="Open Incidents - Pat Morley)" --vertical-label "Number of Tickets" 'DEF:probe1=/data/rrd/pm_open.rrd:inc-all:AVERAGE' 'DEF:probe2=/data/rrd/pm_open.rrd:inc-res:AVERAGE' 'DEF:probe3=/data/rrd/pm_open.rrd:inc-notinten:AVERAGE' 'DEF:probe4=/data/rrd/pm_open.rrd:inc-newinscope:AVERAGE' 'DEF:probe5=/data/rrd/pm_open.rrd:inc-newinscoperes:AVERAGE' 'DEF:probe6=/data/rrd/pm_open.rrd:inc-closedyes:AVERAGE' 'AREA:probe1#FF0000:All Open Tickets:STACK' 'AREA:probe2#00FF00:Resolved:STACK' 'AREA:probe3#0000FF:Not Updated in Ten Days:STACK' 'AREA:probe4#000000:New In Scope:STACK' 'AREA:probe5#FFFF00:New In Scope (Resolved):STACK' 'AREA:probe6#48C4EC:Closed Yesterday:STACK' -w 600 -h 250 -n TITLE:13: --alt-y-grid -l 0 -s now-24w -e now+1d
rrdtool graph $RRDGRAPHREQ -a PNG --title="Open Requests - Pat Morley" --vertical-label "Number of Tickets" 'DEF:probe1=/data/rrd/pm_open.rrd:req-all:AVERAGE' 'DEF:probe2=/data/rrd/pm_open.rrd:req-res:AVERAGE' 'DEF:probe3=/data/rrd/pm_open.rrd:req-notinten:AVERAGE' 'DEF:probe4=/data/rrd/pm_open.rrd:req-newinscope:AVERAGE' 'DEF:probe5=/data/rrd/pm_open.rrd:req-newinscoperes:AVERAGE' 'DEF:probe6=/data/rrd/pm_open.rrd:req-closedyes:AVERAGE' 'AREA:probe1#FF0000:All Open Tickets:STACK' 'AREA:probe2#00FF00:Resolved:STACK' 'AREA:probe3#0000FF:Not Updated in Ten Days:STACK' 'AREA:probe4#000000:New In Scope:STACK' 'AREA:probe5#FFFF00:New In Scope (Resolved):STACK' 'AREA:probe6#48C4EC:Closed Yesterday:STACK' -w 600 -h 250 -n TITLE:13: --alt-y-grid -l 0 -s now-24w -e now+1d
rrdtool graph $RRDGRAPHCHG -a PNG --title="Open Changes - Pat Morley" --vertical-label "Number of Tickets" 'DEF:probe1=/data/rrd/pm_open.rrd:chg-all:AVERAGE' 'DEF:probe2=/data/rrd/pm_open.rrd:chg-res:AVERAGE' 'DEF:probe3=/data/rrd/pm_open.rrd:chg-notinten:AVERAGE' 'DEF:probe4=/data/rrd/pm_open.rrd:chg-newinscope:AVERAGE' 'DEF:probe5=/data/rrd/pm_open.rrd:chg-newinscoperes:AVERAGE' 'DEF:probe6=/data/rrd/pm_open.rrd:chg-closedyes:AVERAGE' 'AREA:probe1#FF0000:All Open Tickets:STACK' 'AREA:probe2#00FF00:Resolved:STACK' 'AREA:probe3#0000FF:Not Updated in Ten Days:STACK' 'AREA:probe4#000000:New In Scope:STACK' 'AREA:probe5#FFFF00:New In Scope (Resolved):STACK' 'AREA:probe6#48C4EC:Closed Yesterday:STACK' -w 600 -h 250 -n TITLE:13: --alt-y-grid -l 0 -s now-24w -e now+1d
echo "" > $MAILFILE
printf "<head><style>
        table {
...
#
for ASSIGNEE in $(cat $OPEN |awk -F'","' '($21~"Incident")&&($4!~"Resolved")&&($23!~".com") { print $25 }'|sed 's/"//g'|sort|uniq|sed '/^$/d')
...
#
for ASSIGNEE in $(cat $OPEN |awk -F'","' '($21~"Change Request") { print $25 }'|sed 's/"//g'|sort|uniq|sed '/^$/d')
	do head -1 $OPEN | awk -F'","' '{ print "<table border='1'><tr bgcolor="999999"><td>"$1"</td><td>"$2"</td><td>"$5"</td><td>"$9"</td><td>"$6"</td><td>"$7"</td><td>"$12"</td><td>"$24"</td></tr>" }'|tee /tmp/$ASSIGNEE.mail;cat $OPEN | awk -F'","' '($21~"Change Request") { print "<tr><td>"$1"</td><td>"$2"</td><td>"$5"</td><td>"$9"</td><td>"$6"</td><td>"$7"</td><td>"$12"</td><td>"$25"</td></tr>" }'|sed 's/"//g' |grep $ASSIGNEE |tee -a /tmp/$ASSIGNEE.mail
        echo "</table></span><br>" |tee -a /tmp/$ASSIGNEE.mail
...
# Send master mail
#
[[ $INCTOT != 0 ]] && mutt -e 'set content_type="text/html"' "paul.ferguson@sungardas.com,patrick.morley@sungardas.com,katie.peacock@sungardas.com,AS.GlobalRSandOpsMgrs@sungardas.com,stephen.carroll@sungardas.com,as.globalrsandopsmgrs@sungardas.com" -s "Daily Pat Morley Open Tickets Report" -a $RRDGRAPHINC -a $RRDGRAPHREQ -a $RRDGRAPHCHG -a $PHTIX -a $PDFFILE < $MAILFILE
#mutt -e 'set content_type="text/html"' "paul.ferguson@sungardas.com" -s "Daily Pat Morley Open Tickets Report" -a $RRDGRAPHINC -a $RRDGRAPHREQ -a $RRDGRAPHCHG -a $PHTIX -a $PDFFILE < $MAILFILE
