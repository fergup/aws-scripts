...
" >> $ECSMAIL
echo "<html><span style='font-family:arial;text-align: center'><table border='1'><tr bgcolor="59FD61"><b><td>Customer SNT</td><td>Customer Name</td><td>CPU in Flex</td><td>Memory in flex (GB)</td><td>Storage in flex (GB)</td><td>Contracted Prod VMs in flex</td><td>Pre-prod VMs in flex</td><td>Potential MRR</td><td>Service Manager</td><td>Days in Flex</td></b></tr>"  >> $ECSMAIL
...
#	YESTMONEY=$(grep $CUSTOMER $ECSMAILYEST |awk -F'<td>' '{ print $4 }'|sed 's/<\/td>//g'|sed 's/£//g')
	YESTMONEY=$(grep $CUSTOMER $ECSMAILYEST |awk -F'>' '{ print $17 }'|sed 's/<\/td//g'|sed 's/£//g')
	YESTFLEX=$(grep $CUSTOMER $ECSMAILYEST |awk -F'>' '{ print $21 }'|sed 's/<\/td//g'|sed 's/£//g')
...
echo "$CUSTOMER,$NAME,$FLEXCPU,$FLEXMEM,$FLEXSTRG,$FLEXVMS,$FLEXPRE,$MONEY,$ACCTMGR,$DAYSFLEX" |awk -F, '($3>0)||($4>0)||($5>0)||($6>0)||($7>0) { print }' >> $ECSOUT
echo "<tr><td>$CUSTOMER</td><td>$NAME</td><td bgcolor="$FLEXCPUCOL">$FLEXCPU</td><td bgcolor="$FLEXMEMCOL">$FLEXMEM</td><td bgcolor="$FLEXSTRGCOL">$FLEXSTRG</td><td bgcolor="$FLEXVMSCOL">$FLEXVMS</td><td bgcolor="$FLEXPRECOL">$FLEXPRE</td><td bgcolor="$DIFFCOL">£${MONEY}</td><td>$ACCTMGR</td><td>$DAYSFLEX</td></tr>" |grep  "bgcolor=red" >> $ECSMAIL
done
# work out how many customers are in flex
CUSTOMERTOT=$(grep "<td>" $ECSMAIL|grep -vc "Customer SNT")
# Final line of table
echo "<tr bgcolor="FFFFCC"><b><td>Total</td><td>Customers in flex:$CUSTOMERTOT</td><td>$FLEXCPUTOT</td><td>${FLEXMEMTOT}GB</td><td>${FLEXSTRGTOT}GB</td><td>$FLEXVMSTOT</td><td>$FLEXPRETOT</td><td>£${MONEYTOT}</td><td>-</td></tr>" >> $ECSMAIL
echo "</table>" >> $ECSMAIL
printf "
...
#mutt -e 'set content_type="text/html"' "paul.ferguson@sungardas.com,paul.labbett@sungardas.com,wayne.hilson@sungardas.com" -s "ECS flex report"  < $ECSMAIL -a $RRDGRAPHMONEY -a $ECSSTAT
#mutt -e 'set content_type="text/html"' "paul.ferguson@sungardas.com,paul.labbett@sungardas.com,wayne.hilson@sungardas.com" -s "ECS flex report"  < $ECSMAIL -a $RRDGRAPHMONEY 
# Tidy up and archive
cp $ECSMAIL  $ARCHDIR/$(basename $ECSMAIL).$DATE
find /data/flex/*Site1* -ctime +3 -exec mv {} /data/archive \;
