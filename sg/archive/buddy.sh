...
NAME=$(echo ${BUDDIES[$NUM]}|awk -F@ '{print $1 }'|awk -F. '{ print $1" "$2 }'|sed -r 's/([a-z]+) ([a-z]+)/\u\1 \u\2/')
	...
	awk -vbuddy=${BUDDIES[$NUM]} -F'","' '($26~buddy) {count++} END {printf "<td font face="arial">"count"</td>"}' $OPEN >> $MAILFILE # Number of tickets assigned to this person
	awk -vbuddy=${BUDDIES[$NUM]} -F'","' '($25~buddy) {count++} END {printf "<td class=xl693002>"count"</td>"}' $OPEN >> $MAILFILE # Number of tickets updated by that person
	awk -vbuddy=${BUDDIES[$NUM]} -F'","' '($13~buddy) {count++} END {printf "<td class=xl693002>"count"</td>"}' $CLOSED >> $MAILFILE # Number of tickets closed by that person
}
buddyor() {
BUDDY1=${BUDDIES[$1]}
BUDDY2=${BUDDIES[$2]}
	printf "<td font face="arial">"$BUDDY1 / $BUDDY2"</td>"	>> $MAILFILE
	awk -vbuddy=${BUDDIES[$NUM]} -F'","' '($26~buddy) {count++} END {printf "<td font face="arial">"count"</td>"}' $OPEN >> $MAILFILE # Number of tickets assigned to this person
	awk -vbuddy=${BUDDIES[$NUM]} -F'","' '($25~buddy) {count++} END {printf "<td class=xl693002>"count"</td>"}' $OPEN >> $MAILFILE # Number of tickets updated by that person
	awk -vbuddy=${BUDDIES[$NUM]} -F'","' '($13~buddy) {count++} END {printf "<td class=xl693002>"count"</td>"}' $CLOSED >> $MAILFILE # Number of tickets closed by that person
}
...
# Team B
buddy 4;buddy 5;buddy 2;buddy 6
printf " </tr>
...
buddy 16;buddy 17;buddy 19;buddy 21
printf " </tr>
...
buddy 2;buddy 38;buddy 39;buddy 40
printf " </tr>
...
printf "</tr>
 <tr height=21 style='height:15.75pt'>" >> $MAILFILE
buddy 45;buddy 46;buddy 47;buddy 2
printf "</tr>
