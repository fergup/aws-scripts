...
NAME=$(echo ${BUDDIES[$NUM]}|awk -F@ '{print $1 }'|awk -F. '{ print $1" "$2 }'|sed -r 's/([a-z]+) ([a-z]+)/\u\1 \u\2/')
	...
	awk -vcountall=0 -vbuddy=${BUDDIES[$NUM]} -F'","' '($26~buddy) {countall++} END {printf "<td font face="arial">"countall"</td>"}' $OPEN >> $MAILFILE # Number of tickets assigned to this person
	awk -vcountup=0 -vbuddy=${BUDDIES[$NUM]} -vtoday="$TODAY" -vyesterday="$YESTERDAY" -F'","' '($25~buddy)&&(($6~today)||($6~yesterday)) {countup++} END {printf "<td class=xl693002>"countup"</td>"}' $OPEN >> $MAILFILE # Number of tickets updated by that person
	awk -vcountcl=0 -vbuddy=${BUDDIES[$NUM]} -F'","' '($13~buddy) {countcl++} END {printf "<td class=xl693002>"countcl"</td>"}' $CLOSED >> $MAILFILE # Number of tickets closed by that person
}
buddydub() {
NUM=$1
NAME=$(echo ${BUDDIES[$NUM]}|awk -F@ '{print $1 }'|awk -F. '{ print $1" "$2 }'|sed -r 's/([a-z]+) ([a-z]+)/\u\1 \u\2/')
...
BUDDY2=$(echo ${BUDDIES[$NUM2]} |awk -F@ '{print $1 }'|awk -F. '{ print $1" "$2 }'|sed -r 's/([a-z]+) ([a-z]+)/\u\1 \u\2/')
	...
	awk -vcountall=0 -vbuddy1=${BUDDIES[$NUM1]} -vbuddy2=${BUDDIES[$NUM2]} -F'","' '($26~buddy1)||($26~buddy2) {countall++} END {printf "<td font face="arial">"countall"</td>"}' $OPEN >> $MAILFILE # Number of tickets assigned to this person
	awk -vcountup=0 -vbuddy1=${BUDDIES[$NUM1]} -vbuddy2=${BUDDIES[$NUM2]}  -vtoday="$TODAY" -vyesterday="$YESTERDAY" -F'","' '($25~buddy1)||($25~buddy2)&&(($6~today)||($6~yesterday)) {countup++} END {printf "<td class=xl693002>"countup"</td>"}' $OPEN >> $MAILFILE # Number of tickets updated by that person
	awk -vcountcl=0 -vbuddy1=${BUDDIES[$NUM1]} -vbuddy2=${BUDDIES[$NUM2]} -F'","' '($13~buddy1)||($13~buddy2) {countcl++} END {printf "<td class=xl693002>"countcl"</td>"}' $CLOSED >> $MAILFILE # Number of tickets closed by that person
}
...
#mutt -e 'set content_type="text/html"' "paul.ferguson@sungardas.com,xande.craveiro-lopes@sungardas.com,jan.clayton-adamson@sungardas.com"  -s "### TEST Daily TOC Buddy List ###" < $MAILFILE 
