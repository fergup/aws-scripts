...
EUFS=$(cat $OPEN|awk -F'","' 'BEGIN{IGNORECASE=1} ($22~"Europe")&&($12~"Adaptiv|Avantgard")&&($18~"Incident")&&($5!~"Resolved")&&($20>1296000) { print $0 }'|wc -l)
EUNOFS=$(cat $OPEN|awk -F'","' 'BEGIN{IGNORECASE=1} ($22~"Europe")&&($12!~"Adaptiv|Avantgard")&&($18~"Incident")&&($5!~"Resolved")&&($20>1296000) { print $0 }'|wc -l)
...
USFS=$(cat $OPEN|awk -F'","' 'BEGIN{IGNORECASE=1} ($22~"US")&&($12~"Sungard Financial")&&($18~"Incident")&&($5!~"Resolved")&&($20>1296000) { print $0 }'|wc -l)
USNOFS=$(cat $OPEN|awk -F'","' 'BEGIN{IGNORECASE=1} ($22~"US")&&($12!~"Sungard Financial|Sungard Investment|Sungard Asset|Sungard Business|Sungard capital")&&($18~"Incident")&&($5!~"Resolved")&&($20>1296000) { print $0 }'|wc -l)
# RRD things
...
mutt -e 'set content_type="text/html"' "paul.ferguson@sungardas.com,pushkar.shirolkar@sungardas.com,jan.clayton-adamson@sungardas.com,kevin.erickson@sungardas.com,as.toc.mgmt@sungardas.com" -s "### Daily Global Open Ticket Report ###"  < $MAILFILE
