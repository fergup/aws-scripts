...
printf "<br><b>Open Incidents older than 30 days by queue</b><br>" >> $MAILFILE
cat $OPEN |awk -F'",' '($21~"Incident")&&($4!~"Resolved") { print $2" "$13"<br>" }' |sort|uniq -c |sort -rn|sed 's/ "/ - /g' |head -15 >> $MAILFILE
echo " " >> $MAILFILE
